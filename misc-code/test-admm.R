...
test_that("Various substitutes give the same value", {
    ...
    bnewC = b_updateC(wvec, uw, rho, cvec3_el, yvec, D, DtDinv, N)##, dimdat, TT, p)
...
test_that("EM algorithm gives same results using ADMM solver or CVXR solver in M step.", {
  ...
  library(devtools)
  compileAttributes("~/repos/flowmix/flowmix")
  load_all('/home/shyun/repos/flowmix/flowmix')

  niter = 1000
  build_all('~/repos/flowmix/flowmix')
  obj_rcpp = profvis({
  ...
  res.admm.old = res.admm

  plot(res.admm$beta[[1]], res.admm.old$beta[[1]])
  abline(0,1)
